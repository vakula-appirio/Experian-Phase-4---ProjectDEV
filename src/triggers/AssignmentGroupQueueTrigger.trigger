/**=====================================================================
 * Appirio, Inc
 * Name: AssignmentGroupQueueTrigger
 * Description: T-309768: round robin for Case
 * Created Date: Aug 28th, 2014
...
trigger AssignmentGroupQueueTrigger on Assignment_Group_Queues__c (before insert, before update) {
  ...
  if (trigger.isUpdate) {
    ...
    Map<String, Group> queuesMap = new Map<String, Group>();     //Queue name --> Queue
    
    for (Group[] q :  [SELECT Name FROM Group WHERE Type= 'Queue']) {
        for (Integer i = 0; i < q.size() ; i++) {
            queuesMap.put(q[i].Name, q[i]);
        }
    }
    ...
    {
        if (queuesMap.containsKey(agq.Name))
        {
            Id qId = queuesMap.get(agq.Name).Id;
